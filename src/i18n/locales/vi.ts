...
const vi: TranslationKeys = {
  ...
  viewTable: 'Bảng',
  viewDiff: 'So sánh',
  noTranslation: 'Chưa dịch',
  emptyField: 'Trống',
  residualCjkBadge: 'còn {n} chữ Hán',
  residualCjkTip: 'Field này đã đánh dấu DỊCH XONG nhưng bản dịch còn {n} ký tự chữ Hán ({pct}% so với gốc) — có thể chưa dịch hết (vd tên biến/nhãn trong schema). Hãy dịch lại hoặc bật Chiến lược B/C để đồng bộ biến.',
  done: 'Xong',
  remaining: 'Còn lại',
  ...
  groupMythic: 'Skill Mythic (Chiến lược A)',
  groupMythicDesc: 'Mô tả + điều kiện kích hoạt của skill Mythic (Auto Database). Agent đọc chúng để quyết định nạp entry nào — không dịch thì chat tiếng Việt sẽ không kích hoạt được entry.',
  msMythicTitle: '🧠 Chiến lược A (Card dùng Mythic)',
  msMythicToggle: 'Dịch skill Mythic + tính lại hash',
  msMythicTip: 'Card Mythic (Auto Database) kích hoạt entry bằng AGENT NGỮ NGHĨA chứ không bằng từ khoá: mỗi entry mang một khối JSON trong phần chú thích, với "mô tả" và "điều kiện kích hoạt". Người chơi chat tiếng Việt mà hai field đó còn tiếng Trung thì Agent không khớp được, entry sẽ không bao giờ được nạp. Chiến lược B và C không đụng tới chúng. Bật A để dịch chúng và tự tính lại mã kiểm tra (hash) — giữ mã cũ là script coi thẻ đã bị sửa hỏng. Chạy riêng hoặc song song với B/C đều được.',
  ...
  msMythicUnsafeTitle: '{count} entry không nên dịch tiêu đề',
  msMythicUnsafeTip: 'Những entry này không ghi sẵn "thời đại" (eras) trong dữ liệu, nên hệ thống phải đoán thời đại từ TỪ KHOÁ trong tiêu đề. Dịch tiêu đề đi là mất từ khoá, entry sẽ bị Agent loại bỏ hoàn toàn. Tiêu đề của chúng được giữ nguyên.',
  msMythicApplied: '🧠 Chiến lược A: đã dịch {meta} field skill, tính lại {hash} mã kiểm tra trên {entries} entry.',
